# Plan 4: Infrastructure & Docs Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add the missing OpenAPI drift check to CI and write the README quickstart so the seed is fully documented and self-verifying.

**Architecture:** The Dockerfile, docker-compose.yml, and CI skeleton already exist and match the spec. Only the OpenAPI diff step is absent from CI, and the README body is a placeholder. Both are standalone changes with no code dependencies.

**Tech Stack:** GitHub Actions, Docker Compose, Markdown.
...
| File | Change |
|---|---|
| `.github/workflows/ci.yml` | **Modify** — add OpenAPI drift check step after `build release` |
| `README.md` | **Replace** — full quickstart replacing the placeholder |

---

## Task 1: OpenAPI drift check in CI

The spec (§11.5) requires: `cargo run --release -- dump-openapi > target/openapi.json && diff docs/openapi.json target/openapi.json`.

Since the release binary is already built in the `build release` step, we run the binary directly. `dump-openapi` calls `AppConfig::from_env()` which requires `EGRAS_DATABASE_URL` and `EGRAS_JWT_SECRET` (≥32 bytes) even though no DB connection is made — the config is parsed before the subcommand branch is reached.
...
Locate the `build release` step (currently last). The new step goes immediately after it.

- [ ] **Step 2: Add the OpenAPI drift check step**
...
The full `steps:` section after the change should look like:
...
- [ ] **Step 4: Smoke-test locally that dump-openapi output matches docs/openapi.json**

```bash
(export $(grep -v '^#' .env | xargs) && cargo run -- dump-openapi) > /tmp/openapi-check.json 2>/dev/null
diff docs/openapi.json /tmp/openapi-check.json && echo "no drift"
```

Expected: `no drift` (no diff output).

If there is a diff, regenerate `docs/openapi.json`:

```bash
(export $(grep -v '^#' .env | xargs) && cargo run -- dump-openapi) > docs/openapi.json 2>/dev/null
git add docs/openapi.json
git commit -m "docs(openapi): sync openapi.json before CI drift check"
```
...
git commit -m "ci: add OpenAPI drift check step"
```
...
The current `README.md` has one sentence: "Quickstart docs arrive with the completion of Plan 3." Replace it entirely.
...
# Basic checks: has headings, has code blocks, no placeholder text
assert '## Quickstart' in text
assert 'seed-admin' in text
assert 'swagger-ui' in text
assert 'Plan 3' not in text, 'placeholder still present'
print('README OK')
"
```
...
```bash
git add README.md
git commit -m "docs: write README quickstart (env vars, docker-compose, seed-admin, tests)"
```
